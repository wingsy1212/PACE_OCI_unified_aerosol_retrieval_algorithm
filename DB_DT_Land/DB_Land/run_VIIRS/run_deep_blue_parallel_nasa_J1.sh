...
if [ $n_files -eq 0 ];
then
  echo No files found for ${l1a_dir}/VJ103MOD.${file_regex}*.
  exit 1
fi

dt=`find -L ${l1a_dir} -maxdepth 1 -name "VJ103MOD.${file_regex}*.nc" -exec basename {} \;|cut -d"." -f 2,3`

machine_name=`uname -n`
case $machine_name in
gs613-crl*)
  /home/asayer/local/parallel-20170322/bin/parallel -j $ncpus ./run_deep_blue_nasa_J1.sh -g {} $root_dir ::: ${dt[*]}
  ;;
tanami*)
  /home/cbettenh/local/parallel-20160622/bin/parallel -j $ncpus ./run_deep_blue_nasa_J1.sh -g {} $root_dir ::: ${dt[*]}
  ;;
sipssci*)
  /home/wvkim/local/parallel-20170822/bin/parallel -j $ncpus ./run_deep_blue_nasa_J1.sh -g {} $root_dir ::: ${dt[*]}  
  ;;
*)
