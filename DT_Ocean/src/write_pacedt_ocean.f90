...
! ---------------------------------------------------------------------
      ...
      subroutine make_nc_Ocean(Save_Lat,Save_Lon,save_SolZen,save_View_angle,&
...
!     Declare output variables
      ...
      real nc_Dust_Flag_ocean(Ret_Xtrack,Ret_Lines)
!     Declare variables for netCDF processing

      integer XL,YL,ZL,li,lj, YY1_new, YY2_new,ZL2,ZL3,ZL4,ZLS
      integer ncid,grpid
      character (len=*), parameter :: nc_name = 'vnpaerdt_output_Ocean.nc'
      real fv3,fv4
       print*,'inside write opening vnpaerdt_output_Ocean.nc'
...
! 
      ...
      nc_Dust_Flag_ocean(1:XL,1:YL)= Save_Dust_Flag_ocean(1:XL,1:YL)
      nc_Dust_Flag_ocean = scale_offset(nc_Dust_Flag_ocean,XX1,SCALE1,OFFSET1,&
                                  fv4,XL,YL) 
...
!             (XX1*ILINE),YY1,(YY2*ILINE),grpid,1,(A_Num_part*ILINE)) 
      ...
      call write_nc_2d(nc_topo_alt,'Topographic_Altitude_Land',&
           XX1,YY1,YY2,grpid,1,A_Num_part)  
      call write_nc_2d(nc_Dust_Flag_Ocean,'Dust_Flag_Ocean',&
             XX1,YY1,YY2,grpid,1,A_Num_part)  
          call check( nf90_inq_ncid(ncid, 'geophysical_data', grpid) )
      ...
      call write_nc_3d(nc_aod_land,'Corrected_Optical_Depth_Land',&
       ...
       call write_nc_3d(nc_numpix_land,'Number_Pixels_Used_Land',&
            XX1,YY1,YY2,ZL3,grpid,A_Num_part) 
       call write_nc_3d(nc_ref_ocean,'Mean_Reflectance_Ocean',&
...
end module write_pace_dt_Ocean
