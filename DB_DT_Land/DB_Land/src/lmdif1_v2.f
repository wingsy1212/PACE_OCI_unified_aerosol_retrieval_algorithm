      subroutine lmdif1_v2(fcn,m,n,x,fvec,tol,info,ipvt,wa,lwa,llim,ulim,ra,fjac)
      integer m,n,info,lwa
      integer ipvt(n)
      double precision tol
      double precision x(n),fvec(m),wa(lwa)
	    double precision diag(n),fjac(m,n),qtf(n),wa1(n),wa2(n),wa3(n),wa4(m)
	    double precision llim(n), ulim(n), ra(n,n)
      external fcn
c     **********
c
c     subroutine lmdif1
...
c
c       subroutine lmdif1(fcn,m,n,x,fvec,tol,info,ipvt,wa,lwa)
c
c     where
c
...
c     **********
      ...
      double precision epsfcn,factor,ftol,gtol,xtol,zero, dpmpar
...
c
      ...
      xtol = tol
      gtol = zero
      epsfcn = 0.000001192	!0.75     !epsfcn = 0.0000001192
      mode = 1
      nprint = 1
      mp5n = m + 5*n  
      call lmdif_v2(fcn,m,n,x,fvec,ftol,xtol,gtol,maxfev,epsfcn,
     *           diag,mode,factor,nprint,info,nfev,fjac,m,
