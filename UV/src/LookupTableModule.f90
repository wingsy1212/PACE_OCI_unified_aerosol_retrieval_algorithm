...
! INPUTS: none
  ...
  CHARACTER(LEN=*), INTENT(IN)  :: sfc_file  
  CHARACTER(LEN=*), INTENT(IN)  :: AIRSCO_clm_file  
    
  ! Temporary variables
  INTEGER(KIND=4) :: unitnum, unitnum1, i, STATUS
  INTEGER(KIND=4) :: ref_highRestmp(nlon_highRes)
    
  !==============================================================================  
    
  OPEN(unit=unitnum,file=sfc_file,status='old')
!  PRINT *, 'Now reading... ', sfc_file
  DO i=1,1080
      READ(unitnum,'(2160I3)',IOSTAT=status) ref_highRestmp
      ref_sfc(i,:) = ref_highRestmp
      IF(status /= 0) THEN
         WRITE( *,'(A)' ) 'Reading sfctype1x1.dat file failed '
         RETURN
      ENDIF
  ENDDO
  Close(unitnum)
  ...
  OPEN(unit=unitnum1,file=AIRSCO_clm_file,status='old')
!  PRINT *, 'Now reading... ', AIRSCO_clm_file
