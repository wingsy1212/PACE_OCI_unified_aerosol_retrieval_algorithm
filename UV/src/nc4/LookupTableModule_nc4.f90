...
!==============================================================================
 ...
 SUBROUTINE Read_ancillaryLUTs(lut_file, rfile)
    ...
    lut_fn = cfg%uv_nc4
    status = nf90_open(lut_fn, nf90_nowrite, nc_id)
    ...
    start  = (/ 1,1 /)
    edges   = (/ nlat_highRes, nlon_highRes /)
    stride = (/ 1,1 /)
    status = nf90_get_var(grp_id, dset_id, ref_sfc, start=start, &
    ...
    start  = (/ 1,1,1 /)
    edges   = (/ nlon, nlat, nmonth /)
    stride = (/ 1,1,1 /)
    status = nf90_get_var(grp_id, dset_id, airsco_cm, start=start, &
                          stride=stride, count=edges)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to read dataset "//trim(dset_name)//": ", status
        return
    ...
    start  = (/ 1,1,1 /)
    edges   = (/ nmonth, nlon, nlat  /)
    stride = (/ 1,1,1 /)
    status = nf90_get_var(grp_id, dset_id, zaer_cm, start=start, &
                          stride=stride, count=edges)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to read dataset "//trim(dset_name)//": ", status
        return
    ...
    edges   = (/ nmonth, nlon, nlat  /)
    stride = (/ 1,1,1 /)
    status = nf90_get_var(grp_id, dset_id, zaer_cm2, start=start, &
                          stride=stride, count=edges)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to read dataset "//trim(dset_name)//": ", status
        return
