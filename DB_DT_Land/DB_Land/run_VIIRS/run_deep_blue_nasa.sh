...
# -- Get OMPS data
  ...
  echo "OMPS file: $omps_file_fin, $time_diff_fin"
...
# by default, assume the anc files are from the same date as the GMTCO files.
	...
	tanami*)
		...
		;;
	windhoek*)
		pre_anc_dir="/tis/modaps/allData/1/DFPITI3NXASM/${anc_year}/"
		post_anc_dir="/tis/modaps/allData/1/DFPITI3NXASM/${anc_year}/"
		;;		
	sipssci*)
    #============for standard retrieval===========
    #===un-comment below for standard retrieval===
    geo_name=`sips_search -a --start ${geo_year0}-${geo_doy0} --end ${geo_year1}-${geo_doy1} --products DFPITI3NXASM |tail -n 2|head -n 1|cut -d "/" -f 9`

    n_geo=`find -L ../input_data -maxdepth 1 -name ${geo_name}|wc -l`

    echo $geo_name    
    if [ $n_geo -lt 1 ];
    then
      sips_search --download=${root_dir}/input_data/ -a --start ${geo_year0}-${geo_doy0} --end ${geo_year1}-${geo_doy1} --products DFPITI3NXASM
    else
      echo 'geos5 file exits, skip sips_search'
    fi
    #=============================================    
...
#   -- increment the date by 1 day to pick up the correct 0z model data.
    ...
    tmp_day=${tmp_date:6:2}
	  ...
	  post_anc_base=GEOS.*.*.inst3_2d_asm_Nx.GEOS*.${geo_year1}${geo_month1}${geo_day1}_${post_model_run}00.*.nc4
    ;;
...
#   creation_dt=`date +%Y%m%d%H%M%S`  #calendar date
  ...
  echo "omps_file=$omps_file_fin" >> $config_file
  ...
  echo "month=$geo_month0" >> $config_file
  echo "day=$geo_day0" >> $config_file
  echo "platform=VIIRS" >> $config_file

#   $root_dir/src/deep_blue $config_file
  case $machine_name in
  gs613-lakechad*)
    $root_dir/src/deep_blue_lake $config_file
    ;;  
  *)
    $root_dir/src/deep_blue $config_file
    ;;
...
#   -- compress output file with h5repack.
    ...
    echo retrieval complete :${output_file}'.'${collection}'.'${creation_dt}'.nc'
    echo '  '
  else
