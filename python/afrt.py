...
class afrt_ocean(object):
    ...
    def __init__(self, nwav, ncase, mode):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(        
                PHI = xr.DataArray(np.zeros((NRAA), dtype=np.float32), dims=["dim_raa"]),
                THET = xr.DataArray(np.zeros((NVZA), dtype=np.float32), dims=["dim_vza"]),
                THET0 = xr.DataArray(np.zeros((NSZA), dtype=np.float32), dims=["dim_sza"]),
                WAVE = xr.DataArray(np.zeros((nwav), dtype=np.float32), dims=["dim_wav"]),
                ALBEDO_R_RAY = xr.DataArray(np.zeros((NSZA,nwav), dtype=np.float32), 
                                    dims=["dim_sza","dim_wav"]),
                ALBEDO_T_RAY = xr.DataArray(np.zeros((NSZA,nwav), dtype=np.float32), 
                                    dims=["dim_sza","dim_wav"]),
                REF_RAYALL = xr.DataArray(np.zeros((nwav,NSZA,NVZA,NRAA), dtype=np.float32), 
                                    dims=["dim_wav","dim_sza","dim_vza","dim_raa"]),
                EXT = xr.DataArray(np.zeros((nwav,ncase), dtype=np.float32), 
                                    dims=["dim_wav","dim_case"]),
                RGS = xr.DataArray(np.zeros((ncase), dtype=np.float32), 
                                    dims=["dim_case"]),
                SIGMA = xr.DataArray(np.zeros((ncase), dtype=np.float32), 
                                    dims=["dim_case"]),
                MOMENTS = xr.DataArray(np.zeros((NMOMENTS,ncase), dtype=np.float32), 
                                    dims=["dim_mom","dim_case"]),
                CCN = xr.DataArray(np.zeros((ncase), dtype=np.float32), 
                                    dims=["dim_case"]),
                BACKSCTT = xr.DataArray(np.zeros((nwav,ncase), dtype=np.float32), 
                                    dims=["dim_wav","dim_case"]),
                ASSYM = xr.DataArray(np.zeros((nwav,ncase), dtype=np.float32), 
                                    dims=["dim_wav","dim_case"]),
                ALBEDO = xr.DataArray(np.zeros((nwav,ncase), dtype=np.float32), 
                                    dims=["dim_wav","dim_case"]),
                ALBEDO_R = xr.DataArray(np.zeros((ncase,nwav,NAOT,NSZA), dtype=np.float32), 
                                    dims=["dim_case","dim_wav","dim_aot","dim_sza"]),
                ALBEDO_T = xr.DataArray(np.zeros((ncase,nwav,NAOT,NSZA), dtype=np.float32), 
                                    dims=["dim_case","dim_wav","dim_aot","dim_sza"]),
                TAUA = xr.DataArray(np.zeros((NAOT,nwav,ncase), dtype=np.float32), 
                                    dims=["dim_aot","dim_wav","dim_case"]),
                AINT = xr.DataArray(np.zeros((ncase,nwav,NAOT,NSZA,NVZA,NRAA), dtype=np.float32), 
                                    dims=["dim_case","dim_wav","dim_aot","dim_sza","dim_vza","dim_raa"]),
                EFFRAD = xr.DataArray(np.zeros((ncase), dtype=np.float32), 
                                    dims=["dim_case"])
            )
        )
...
class afrt_land(object):
    ...
    def __init__(self):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(        
                PHI = xr.DataArray(np.zeros((NLRAA), dtype=np.float32), dims=["dim_raa"]),
                THET = xr.DataArray(np.zeros((NLVZA), dtype=np.float32), dims=["dim_vza"]),
                THET0 = xr.DataArray(np.zeros((NLSZA), dtype=np.float32), dims=["dim_sza"]),
                MU0 = xr.DataArray(np.zeros((NLSZA), dtype=np.float32), dims=["dim_sza"]),
                WAVE = xr.DataArray(np.zeros((1), dtype=np.float32), dims=["dim_wav"]),
                ROD = xr.DataArray(np.zeros((1), dtype=np.float32), dims=["dim_wav"]),
                GOD = xr.DataArray(np.zeros((1), dtype=np.float32), dims=["dim_wav"]),
                SSA = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                QEXT = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                BEXT = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                VEXT = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                MEXT = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                MASSCOEF = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                OPTH = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                EXTNORM = xr.DataArray(np.zeros((NLTABLE,NLTAU), dtype=np.float32), 
                            dims=["dim_table","dim_tau"]),
                SBAR = xr.DataArray(np.zeros((NLTABLE,NLTAU,NLSZA), dtype=np.float32), 
                            dims=["dim_table","dim_tau","dim_sza"]),
                INT = xr.DataArray(np.zeros((NLTABLE,NLTAU,NLSZA,NLVZA,NLRAA), dtype=np.float32), 
                            dims=["dim_table","dim_tau","dim_sza","dim_vza","dim_raa"]),
                FD = xr.DataArray(np.zeros((NLTABLE,NLTAU,NLSZA), dtype=np.float32), 
                            dims=["dim_table","dim_tau","dim_sza"]),
                T = xr.DataArray(np.zeros((NLTABLE,NLTAU,NLSZA,NLVZA), dtype=np.float32), 
    ...
    def __init__(self):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(        
                ...
                THET = xr.DataArray(np.zeros((NVZA), dtype=np.float32), dims=["dim_vza"]),
                THET0 = xr.DataArray(np.zeros((NSZA), dtype=np.float32), dims=["dim_sza"]),
                WAVE = xr.DataArray(np.zeros((NWAV_UV), dtype=np.float32), dims=["dim_wav"]),
                TAUA = xr.DataArray(np.zeros((NAOT,NWAV_UV,NCASES), dtype=np.float32), 
                            dims=["dim_aot","dim_wav","dim_case"]),
                ALBEDO = xr.DataArray(np.zeros((NWAV_UV,NCASES,NHEIGHT,NOMEGA), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_height","dim_omega"]),
                EXT = xr.DataArray(np.zeros((NWAV_UV,NCASES,NHEIGHT,NOMEGA), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_height","dim_omega"]),
                REF_RAYALL = xr.DataArray(np.zeros((NHEIGHT,NWAV_UV,NSZA,NVZA,NRAA,NWS,NOMEGA), dtype=np.float32), 
                            dims=["dim_height","dim_wav","dim_sza","dim_vza","dim_raa","dim_wspd","dim_omega"]),
                AINT = xr.DataArray(np.zeros((NCASES,NWAV_UV,NAOT,NSZA,NVZA,NRAA,NWS,NHEIGHT,NOMEGA), dtype=np.float32), 
    ...
    def __init__(self):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(        
                ...
                ALBEDO_T_RAY = xr.DataArray(np.zeros((NSZA,NWAV), dtype=np.float32), 
                            dims=["dim_sza","dim_wav"]),
                REF_RAYALL = xr.DataArray(np.zeros((NWAV,NSZA,NVZA,NRAA,NWS), dtype=np.float32), 
                            dims=["dim_wav","dim_sza","dim_vza","dim_raa","dim_wspd",]),
                EXT = xr.DataArray(np.zeros((NWAV,NCASES,NWS), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_wspd"]),
                RGS = xr.DataArray(np.zeros((NCASES), dtype=np.float32), 
                            dims=["dim_case"]),
                SIGMA = xr.DataArray(np.zeros((NCASES), dtype=np.float32), 
                            dims=["dim_case"]),
                MOMENTS = xr.DataArray(np.zeros((NMOMENTS,NCASES,NWS), dtype=np.float32), 
                            dims=["dim_mom","dim_case","dim_wspd"]),
                CCN = xr.DataArray(np.zeros((NCASES,NWS), dtype=np.float32), 
                            dims=["dim_case","dim_wspd"]),
                BACKSCTT = xr.DataArray(np.zeros((NWAV,NCASES,NWS), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_wspd"]),
                ASSYM = xr.DataArray(np.zeros((NWAV,NCASES,NWS), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_wspd"]),
                ALBEDO = xr.DataArray(np.zeros((NWAV,NCASES,NWS), dtype=np.float32), 
                            dims=["dim_wav","dim_case","dim_wspd"]),
                ALBEDO_R = xr.DataArray(np.zeros((NCASES,NWAV,NAOT,NSZA,NWS), dtype=np.float32), 
                            dims=["dim_case","dim_wav","dim_aot","dim_sza","dim_wspd"]),
                ALBEDO_T = xr.DataArray(np.zeros((NCASES,NWAV,NAOT,NSZA,NWS), dtype=np.float32), 
                            dims=["dim_case","dim_wav","dim_aot","dim_sza","dim_wspd"]),
                TAUA = xr.DataArray(np.zeros((NAOT,NWAV,NCASES), dtype=np.float32), 
                            dims=["dim_aot","dim_wav","dim_case"]),
                AINT = xr.DataArray(np.zeros((NCASES,NWAV,NAOT,NSZA,NVZA,NRAA,NWS), dtype=np.float32), 
                            dims=["dim_case","dim_wav","dim_aot","dim_sza","dim_vza","dim_raa","dim_wspd"]),
                EFFRAD = xr.DataArray(np.zeros((NCASES), dtype=np.float32), 
                            dims=["dim_case"])
            )
        )
    ...
    def __init__(self):
        ...
        self.ds = xr.Dataset(
            data_vars=dict(        
                ...
                THET0 = xr.DataArray(np.zeros((NLSZA), dtype=np.float32), dims=["dim_sza"]),
                MU0 = xr.DataArray(np.zeros((NLSZA), dtype=np.float32), dims=["dim_sza"]),
                WAVE = xr.DataArray(np.zeros((NLWAV), dtype=np.float32), dims=["dim_wav"]),
                ROD = xr.DataArray(np.zeros((NLWAV), dtype=np.float32), dims=["dim_wav"]),
                GOD = xr.DataArray(np.zeros((NLWAV), dtype=np.float32), dims=["dim_wav"]),
                OPTH = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                MASSCOEF = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                EXTNORM = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                SSA = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                QEXT = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                BEXT = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                VEXT = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                MEXT = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau"]),
                SBAR = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU,NLSZA), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau","dim_sza"]),
                INT = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU,NLSZA,NLVZA,NLRAA), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau","dim_sza","dim_vza","dim_raa"]),
                FD = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU,NLSZA), dtype=np.float32), 
                        dims=["dim_table","dim_wav","dim_tau","dim_sza"]),
                T = xr.DataArray(np.zeros((NLTABLE,NLWAV,NLTAU,NLSZA,NLVZA), dtype=np.float32), 
    ...
    def read_file(self):    
        ...
        if self.name == "gas_coeffs":
            ...
            dgc = xr.Dataset(
                data_vars=dict(        
                    ...
                    WAVE = xr.DataArray(np.zeros((NBANDS), dtype=np.float32), dims=["dim_bands"]),
                    MOL = xr.DataArray(np.zeros((NBANDS), dtype=np.float32), dims=["dim_bands"]),
                    OPT_O3_CLIM = xr.DataArray(np.zeros((NBANDS), dtype=np.float32), dims=["dim_bands"]),
                    OPT_H2O_CLIM = xr.DataArray(np.zeros((NBANDS), dtype=np.float32), dims=["dim_bands"]),
                    OPT_CO2_CLIM = xr.DataArray(np.zeros((NBANDS), dtype=np.float32), dims=["dim_bands"]),
                    O3_COEF = xr.DataArray(np.zeros((2,NBANDS), dtype=np.float32), dims=["dim_coef2","dim_bands"]),
                    H2O_COEF = xr.DataArray(np.zeros((3,NBANDS), dtype=np.float32), dims=["dim_coef3","dim_bands"]),
                )
            )
        ...
        if self.name == "ext_coeffs":
            ...
            dec = xr.Dataset(data_vars=dict(
                EXT_COEFFS = xr.DataArray(np.zeros((NCASES,NWS), dtype=np.float32), 
                                          dims=["dim_case","dim_wspd"])))
            for iw in range(NWS):
